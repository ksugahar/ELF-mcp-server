"""Raw ELF600 examples directory access — bundled examples_dump.json (332 files).

Generated by ``scripts/crawl_examples.py`` from ``C:/ELF600/examples/``
(.mai analysis input + .mei mesh script + .txt + .props + .model).
Skips binary .meg / .mag.
...
def load_examples_dump() -> dict[str, dict[str, Any]]:
    """Load bundled examples_dump.json once and cache.
...
def list_examples(
    ...
    """List bundled example files, optionally filtered.
        ...
        if len(candidates) == 1:
            ...
            return {
                ...
                "error": f"not found (try one of: {candidates[:5]})" if candidates
                         else "not found in examples_dump (use elf_examples_index to list available files)",
            }
